...
@section('content')
    ...
    <!-- Live Court Status (Livewire Component) -->
    <div style="margin-bottom: 2rem;">
        <livewire:court-status />
    </div>
    ...
    <div class="grid grid-cols-2">
        ...
        <div class="card">
            <div class="card-header">
                <h2 class="card-title"><i class="fas fa-calendar-alt"
                        style="color: var(--primary); margin-right: 0.5rem;"></i> Upcoming Bookings</h2>
            ...
            @if($upcomingBookings->count() > 0)
                @foreach($upcomingBookings as $booking)
                    <div
                        style="display: flex; align-items: center; gap: 1rem; padding: 1rem; background: var(--bg-card-hover); border-radius: var(--radius-md); margin-bottom: 0.75rem;">
                        <div
                            style="width: 50px; height: 50px; background: var(--gradient-primary); border-radius: var(--radius-md); display: flex; align-items: center; justify-content: center; color: white; font-weight: 700;">
                            {{ $booking->booking_date->format('d') }}
                        </div>
                        <div style="flex: 1;">
                            <div style="font-weight: 600;">{{ $booking->court->court_name }}</div>
                            <div style="font-size: 0.875rem; color: var(--text-secondary);">
                                {{ $booking->booking_date->format('D, M d') }} •
                                {{ \Carbon\Carbon::parse($booking->start_time)->format('g:i A') }} -
                                {{ \Carbon\Carbon::parse($booking->end_time)->format('g:i A') }}
                            </div>
                        </div>
                        <span class="badge badge-{{ $booking->display_status }}">{{ ucfirst($booking->display_status) }}</span>
                    </div>
                @endforeach
            @else
                <div class="empty-state">
                    ...
                    <a href="{{ route('bookings.create') }}" class="btn btn-primary btn-sm" style="margin-top: 1rem;">Book a
                        Court</a>
                </div>
            @endif
        </div>
        ...
        <div>
            <div class="card" style="margin-bottom: 1.5rem;">
                <div class="card-header">
                    <h2 class="card-title"><i class="fas fa-bolt" style="color: var(--accent); margin-right: 0.5rem;"></i>
                        Quick Actions</h2>
                </div>
                ...
                @if($recentBookings->count() > 0)
                    @foreach($recentBookings->take(4) as $booking)
                        <div
                            ...
                            <span class="badge badge-{{ $booking->display_status }}"
                                style="min-width: 80px; justify-content: center;">{{ ucfirst($booking->display_status) }}</span>
