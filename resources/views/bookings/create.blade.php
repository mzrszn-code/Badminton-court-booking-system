        ...
        <div class="card">
            <form method="POST" action="{{ route('bookings.store') }}" id="bookingForm">
                ...
                <div class="form-group">
                    ...
                    <div id="courtCards"
                        style="display: grid; gap: 0.75rem; margin-bottom: 1rem; max-height: 280px; overflow-y: auto; padding-right: 0.5rem;">
                        @foreach($courts as $court)
                            <div class="court-card {{ ($selectedCourt && $selectedCourt->id == $court->id) ? 'selected' : '' }}"
                                data-court-id="{{ $court->id }}" data-rate="{{ $court->hourly_rate }}"
                                ...
                                <div style="display: flex; justify-content: space-between; align-items: center;">
                                    <div>
                                        ...
                                        <div style="font-size: 0.75rem; color: var(--text-secondary);">
                                            {{ ucfirst($court->court_type) }} • {{ $court->location }}
                                        </div>
                                    </div>
                                    <div style="text-align: right;">
                                        <div style="font-weight: 700; color: var(--primary);">RM
                    ...
                    <input type="hidden" name="court_id" id="courtId" value="{{ $selectedCourt ? $selectedCourt->id : '' }}"
                        required>
                </div>

                <div class="form-group">
                    <label class="form-label">Booking Date *</label>
                    <input type="date" name="booking_date" id="bookingDate" class="form-control" value="{{ $selectedDate }}"
        ...
        <div>
            <div class="card">
                <div class="card-header">
                    <h2 class="card-title"><i class="fas fa-clock" style="color: var(--primary); margin-right: 0.5rem;"></i>
                ...
                <div
                    style="background: var(--bg-card-hover); padding: 0.75rem 1rem; border-radius: var(--radius-sm); margin-bottom: 1rem; font-size: 0.875rem;">
            ...
            <div class="card" style="margin-top: 1.5rem;" id="bookingSummary">
                <div class="card-header">
                    <h2 class="card-title"><i class="fas fa-receipt"
                            style="color: var(--accent); margin-right: 0.5rem;"></i> Booking Summary</h2>
                </div>
    ...
    @push('scripts')
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                ...
                // Set minimum date to today and enforce it
                ...
                // Date change - validate and fetch availability
                bookingDate.addEventListener('change', function () {
                    // Prevent past dates
                    if (this.value < today) {
                        this.value = today;
                        alert('Cannot book for past dates. Date has been reset to today.');
                    }
                    resetSlotSelection();
                    fetchAvailability();
                });
                ...
                function calculateHours(startTime, endTime) {
                    ...
                    // Normalize: hours 0-3 are treated as 24-27 (continuation of previous day)
                ...
                function updateSummary() {
                    ...
                    // Calculate hours with overnight normalization (0-3 AM = 24-27)
                    const hours = calculateHours(selectedStartSlot, selectedEndSlot);
                ...
                function updateSubmitButton() {
                    ...
                    if (isValid) {
                        // Use overnight-aware hour calculation
