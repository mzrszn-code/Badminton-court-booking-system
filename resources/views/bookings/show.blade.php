...
@section('content')
    ...
    @php
        // Calculate hours with overnight normalization (0-3 AM treated as 24-27)
    ...
    <div class="grid grid-cols-2" style="grid-template-columns: 1fr 350px;">
        ...
        <div class="card">
            <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 2rem;">
                ...
                <span class="badge badge-{{ $booking->display_status }}" style="font-size: 1rem; padding: 0.5rem 1rem;">
                    {{ ucfirst($booking->display_status) }}
                </span>
            </div>
            ...
            <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 1.5rem; margin-bottom: 2rem;">
                <div style="padding: 1.5rem; background: var(--bg-card-hover); border-radius: var(--radius-md);">
                    <div
                        style="font-size: 0.75rem; color: var(--text-secondary); text-transform: uppercase; letter-spacing: 0.05em; margin-bottom: 0.5rem;">
                        Court</div>
                ...
                <div style="padding: 1.5rem; background: var(--bg-card-hover); border-radius: var(--radius-md);">
                    ...
                    <div style="font-size: 0.875rem; color: var(--text-secondary);">
                        {{ $booking->booking_date->format('F d, Y') }}</div>
                </div>
                ...
                <div style="padding: 1.5rem; background: var(--bg-card-hover); border-radius: var(--radius-md);">
                    ...
                    <div style="font-size: 0.875rem; color: var(--text-secondary);">@ RM
                        {{ number_format($booking->court->hourly_rate, 2) }}/hour</div>
                </div>
            </div>
            ...
            @if($booking->notes)
                <div style="margin-bottom: 2rem;">
                    ...
                    <p
                        style="color: var(--text-secondary); background: var(--bg-card-hover); padding: 1rem; border-radius: var(--radius-md);">
                        {{ $booking->notes }}</p>
                </div>
            @endif
            ...
            @if($booking->checkin && $booking->checkin->is_checked_in)
                <div class="alert alert-success">
                    ...
                    <div>
                        ...
                        <p style="font-size: 0.875rem; margin-top: 0.25rem;">Checked in at
                            {{ $booking->checkin->checkin_time->format('g:i A') }}</p>
                    </div>
                </div>
            @endif
        </div>
        ...
        <div>
            @if($booking->status === 'approved' && $booking->checkin && $paymentCompleted)
                <div class="card" style="text-align: center;">
                    ...
                    <div
                        style="background: white; padding: 1.5rem; border-radius: var(--radius-md); display: inline-block; margin-bottom: 1.5rem;">
            ...
            @elseif($booking->status === 'approved' && $booking->payment && $booking->payment->payment_status === 'pending')
                ...
                <div class="card" style="text-align: center;">
                    <div
                        style="width: 80px; height: 80px; background: var(--gradient-accent); border-radius: 50%; display: flex; align-items: center; justify-content: center; margin: 0 auto 1.5rem;">
                    ...
                    <p style="color: var(--text-secondary);">Your payment is being verified by admin. QR code will be available
                        once verified.</p>
                    <div
                        style="margin-top: 1rem; padding: 0.75rem; background: var(--bg-card-hover); border-radius: var(--radius-sm);">
            ...
            @elseif($booking->status === 'approved' && (!$booking->payment || $booking->payment->payment_status === 'rejected'))
                ...
                <div class="card" style="text-align: center;">
                    ...
                    @endif
                    <a href="{{ route('payments.show', $booking) }}" class="btn btn-primary"
            ...
            @elseif($booking->status === 'pending')
                <div class="card" style="text-align: center;">
                    ...
                    <p style="color: var(--text-secondary);">Your booking is pending admin approval. You'll receive a
                        notification once it's approved.</p>
                </div>
            @elseif($booking->status === 'rejected')
                <div class="card" style="text-align: center;">
                    <div
                        style="width: 80px; height: 80px; background: var(--danger); border-radius: 50%; display: flex; align-items: center; justify-content: center; margin: 0 auto 1.5rem;">
                    ...
                    <p style="color: var(--text-secondary);">Unfortunately, this booking was rejected. Please try booking a
            ...
            @elseif($booking->status === 'cancelled')
                <div class="card" style="text-align: center;">
                    <div
                        style="width: 80px; height: 80px; background: var(--text-muted); border-radius: 50%; display: flex; align-items: center; justify-content: center; margin: 0 auto 1.5rem;">
