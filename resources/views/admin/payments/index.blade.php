                ...
                @forelse($payments as $payment)
                    <tr>
                        ...
                        <td>
                            ...
                            <div style="font-size: 0.75rem; color: var(--text-secondary);">{{ $payment->booking->user->email }}</div>
                            <div style="font-size: 0.75rem; color: var(--text-secondary);">{{ $payment->booking->user->phone ?? 'No phone' }}</div>
                        </td>
                        <td>
                            <div style="font-weight: 500;">#{{ $payment->booking_id }} - {{ $payment->booking->court->court_name }}</div>
                        ...
                        <td style="font-weight: 700; color: var(--primary);">RM {{ number_format($payment->amount, 2) }}</td>
                            ...
                            @if($payment->payment_status === 'pending')
                                <div style="display: flex; gap: 0.5rem; flex-wrap: wrap;">
                                    @if($payment->proof_image)
                                        <a href="{{ asset('storage/' . $payment->proof_image) }}" target="_blank" class="btn btn-sm btn-secondary" title="View Receipt">
                ...
                @empty
                    <tr>
                        <td colspan="9" style="text-align: center; padding: 3rem;">
                            <i class="fas fa-credit-card" style="font-size: 2rem; color: var(--text-muted); margin-bottom: 1rem; display: block;"></i>
