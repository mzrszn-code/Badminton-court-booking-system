...
@section('content')
    ...
    <div class="grid grid-cols-3">
        ...
        <div class="card">
            <div class="card-header">
                <h2 class="card-title"><i class="fas fa-bolt" style="color: var(--accent); margin-right: 0.5rem;"></i> Quick
                    Access</h2>
            </div>

            <div style="display: grid; gap: 0.75rem;">
                <a href="{{ route('admin.payments.index') }}?status=pending" class="btn btn-primary"
                    style="justify-content: flex-start;">
                    <i class="fas fa-credit-card" style="width: 20px;"></i> Pending Payments
        ...
        <div class="card">
            <div class="card-header">
                <h2 class="card-title"><i class="fas fa-chart-bar" style="color: var(--primary); margin-right: 0.5rem;"></i>
                    Peak Hours</h2>
            </div>
            ...
            @if($peakHours->count() > 0)
                @foreach($peakHours as $peak)
                    <div style="display: flex; align-items: center; gap: 1rem; margin-bottom: 0.75rem;">
                        ...
                        <div
                            style="flex: 1; height: 24px; background: var(--bg-card-hover); border-radius: var(--radius-sm); overflow: hidden;">
                            <div
                                style="height: 100%; width: {{ min(100, ($peak->count / max($peakHours->max('count'), 1)) * 100) }}%; background: var(--gradient-primary); border-radius: var(--radius-sm);">
                            </div>
                        </div>
        ...
        <div class="card">
            <div class="card-header">
                <h2 class="card-title"><i class="fas fa-trophy" style="color: var(--success); margin-right: 0.5rem;"></i>
                    Popular Courts</h2>
            </div>
            ...
            @if($popularCourts->count() > 0)
                @foreach($popularCourts as $index => $court)
                    <div
                        ...
                        <div
                            style="width: 32px; height: 32px; background: {{ $index === 0 ? 'var(--gradient-accent)' : 'var(--bg-card-hover)' }}; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-weight: 700; font-size: 0.875rem; color: {{ $index === 0 ? 'white' : 'var(--text-secondary)' }};">
                            {{ $index + 1 }}
                        </div>
                        <div style="flex: 1;">
                            <div style="font-weight: 600;">{{ $court->court_name }}</div>
                            <div style="font-size: 0.75rem; color: var(--text-secondary);">{{ $court->bookings_count }} bookings
                            </div>
                        </div>
                    </div>
                @endforeach
    ...
    <div class="grid grid-cols-2" style="margin-top: 1.5rem;">
        ...
        <div class="card">
            <div class="card-header">
                <h2 class="card-title"><i class="fas fa-calendar"
                        style="color: var(--secondary); margin-right: 0.5rem;"></i> Recent Bookings</h2>
                    ...
                    <tbody>
                        @forelse($recentBookings as $booking)
                            <tr>
                                ...
                                <td><span
                                        class="badge badge-{{ $booking->display_status }}">{{ ucfirst($booking->display_status) }}</span>
                                </td>
                                <td>
        ...
        <div class="card">
            <div class="card-header">
                <h2 class="card-title"><i class="fas fa-wrench" style="color: var(--danger); margin-right: 0.5rem;"></i>
                    Active Maintenances</h2>
            ...
            @if($activeMaintenances->count() > 0)
                @foreach($activeMaintenances as $maintenance)
                    <div
                        style="padding: 1rem; background: var(--bg-card-hover); border-radius: var(--radius-md); margin-bottom: 0.75rem; border-left: 3px solid var(--danger);">
